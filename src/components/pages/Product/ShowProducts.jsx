...
import { FaBoxOpen,FaShoppingCart, FaEye ,FaArrowLeft,FaCheckCircle} from 'react-icons/fa';
      ...
      <div className="text-center py-16">
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {products.map((product, index) => (
          <Card key={index} className="w-full hover:shadow-xl transition-shadow duration-300 border border-gray-100">
          ...
          <CardBody className="pt-3 pb-1">
            <div className="flex justify-between items-start mb-2">
              <Typography variant="h6" className="font-bold text-gray-900 line-clamp-1">
                {product.title}
              </Typography>
            ...
            <div className="flex items-center text-sm text-gray-500 mb-2">
              ...
              <span>{product.availableQuantity > 5 ? 'In Stock' : 'Low Stock'}</span>
            </div>
          </CardBody>
          <CardFooter className="flex flex-col gap-3 pt-1">
            ...
            <Link to={`/show-products/${category}/${product.title}`} className="w-full">
              <Button
                variant="outlined"
                fullWidth
