          ...
          <div className="mt-6 sm:mt-8 md:gap-6 lg:flex lg:items-start xl:gap-8">
            <div className="mx-auto w-full flex-none lg:max-w-2xl xl:max-w-4xl">
              ...
              <div className="block sm:hidden">
                {cart.length > 0 ? (
                  cart.map((item) => (
                    <div key={item.id} className="flex items-center border-b py-4 space-x-4 hover:bg-gray-50 transition-all duration-200">
                      <img className="h-16 w-16 object-cover rounded-md shadow-md" src={item.images[0]} alt={item.title} />
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-gray-800">{item.title.split(" ").slice(0, 2).join(" ")}</h3>
                        <div className="flex justify-between mt-2 text-sm text-gray-600">
                          <span>Price: {item.price} EGP</span>
                          <div className="flex items-center gap-2">
                            <IconButton color="red" onClick={() => increaseQty(item.id)} size="sm">
                              +
                            </IconButton>
                            <h1 className="font-medium">{item.quantity}</h1>
                            <IconButton color="red" onClick={() => decreaseQty(item.id)} size="sm">
                              -
                            </IconButton>
                          </div>
                        </div>
                        <div className="mt-2 flex justify-between items-center text-sm text-gray-600">
                          <span>Subtotal: {item.subtotal.toFixed(2)} EGP</span>
                          <button className="text-red-600 hover:underline font-medium" onClick={() => removeFromCart(item.id)}>
                            Remove
                          </button>
                        </div>
                      </div>
